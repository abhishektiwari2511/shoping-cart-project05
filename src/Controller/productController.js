...
const getbyquery = async function(req,res){
    ...
    if(priceGreaterThan){
        if(!isvalidPrice(priceGreaterThan)) return res.status(400).send({ stastus: false, message: "Invalid pirce format!" });
        filter.price={$gt:priceGreaterThan}
    }
    if(priceLessThan){
        if(!isvalidPrice(priceLessThan)) return res.status(400).send({ stastus: false, message: "Invalid pirce format!" });
        filter.price={$lt:priceLessThan}
    }
    if(priceGreaterThan&&priceLessThan){
        if(!isvalidPrice(priceGreaterThan) || !isvalidPrice(priceGreaterThan)) return res.status(400).send({ stastus: false, message: "Invalid pirce format!" });
        if(priceGreaterThan<priceLessThan){filter.price={$gt:priceGreaterThan,$lt:priceLessThan}}
        else return res.status(400).send({ stastus: false, message: "priceGreaterThan and priceLessThan is always higher or lower than pirce not equals to price" });
    }
    ...
    return res.status(200).send({status:true, message : 'Success', message:finaldata})   
}   
...
const getbyparams = async function (req,res){
    ...
    const getdata = await productModel.findOne({_id:productId,isDeleted:false})
     if (!getdata || getdata.isDeleted == true){return res.status(404).send({status:false,message:"No data found from this Id or it is deleted"})}
      ...
      const updateProducts = async function (req, res) {
        try {
          ...
          let files = req.files;
            ...
            if (!checkProduct || checkProduct.isDeleted == true) {
                return res.status(404).send({ status: false, message: "product are not found or its already deleted" })
            }
          ...
          if(data.hasOwnProperty("currencyId")){
           ...
           if(data.hasOwnProperty("currencyFormat")){
            ...
            if(data.hasOwnProperty("deletedAt")){
                if (!isValid(deletedAt) || !isvalidPrice(deletedAt)) return res.status(400).send({ status: false, message: "installments must be valid" })
